...
Recently research has been done into a new type of programming language, they are called probabilistic programming languages (PPL). There are PPLs that extend existing languages, including C, Java, Python, Haskell, and Scheme. Obviously these base languages differ in terms of their paradigms and their execution speed and what we attempt in this project is to make a compiler from a high level probabilistic programming language to a lower level one. We do this both for the performance increase of using a lower level language and also so we can perform some probabilistic optimizations along the way.
...
Usually a probabilistic programming language is based on an existing language rather than being a completely new one, examples include: Venture (Scheme), Anglican (Scheme), IBAL (OCaml), Infer.NET (.NET), PSQL (SQL), FACTORIE (Scala), Alchemy (C++), Church (Scheme), Stan (none). More are being designed constantly as people discover new ways of using them.
